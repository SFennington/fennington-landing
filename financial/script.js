...
function transactionRow(tx) {
  ...
  return `<tr data-id="${escapeAttr(tx.id)}" class="tx-row ${tx.needsReview ? "needs-review" : ""}">
    <td class="date-cell" data-label="Date"><span class="date-value" aria-label="Imported transaction date">${escapeHtml(tx.date || "")}</span></td>
    <td class="tx-description-cell" data-label="Merchant">
    ...
    <td class="category-cell" data-label="Category">${transactionCategoryCombobox(tx)}</td>
    <td class="flow-cell" data-label="Flow">${flowStatusHtml(tx)}</td>
    <td class="confidence-cell" data-label="Confidence"><div class="confidence-meter" aria-label="${confidence} percent confidence"><span style="width:${confidence}%"></span></div><div class="confidence-meta"><strong>${confidence}%</strong><small>${escapeHtml(source)}</small></div></td>
    <td class="type-cell" data-label="Type"><select class="type-select type-${escapeAttr(type)}" data-field="type" aria-label="Transaction type">${typeOptions}</select></td>
