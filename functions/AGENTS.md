...
- **Fulfillment monitoring** (`requireAdmin`): `POST /admin/jobs/monitor-fulfillment` (on-demand run of the reconciliation job, also runs on the `scheduledMonitorStripeFulfillment` schedule every 30 minutes), `GET /admin/support-alerts`, `POST /admin/support-alerts/:alertId/resolve`, `POST /admin/support-alerts/:alertId/resend-access`. `runFulfillmentMonitor()` cross-checks recent Stripe checkout sessions against `digitalPurchases`/`downloadTokens` to catch payments that never got fulfilled, fulfilled purchases whose access email never sent, and unused access links that expired — writes to `supportAlerts` and emails `getAdminEmails()` once per newly-found issue via the same Resend pattern as digital-product fulfillment email.
